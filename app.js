// ---- Web Calendario: bootstrap + error context (Sentry-friendly) ----
(function() {
  if (window.__WC_APP_BOOTSTRAPPED__) return;
  window.__WC_APP_BOOTSTRAPPED__ = true;

  window.__WC_APP_STATE__ = window.__WC_APP_STATE__ || {};
  window.__WC_APP_STATE__.build = window.__WC_APP_STATE__.build || "20260128210911";
  window.__WC_APP_STATE__.schema = window.__WC_APP_STATE__.schema || 1;
  window.__WC_APP_STATE__.path = window.location && window.location.pathname;

  function storeLastError(payload) {
    try {
      const key = "__wc_last_error__";
      const cur = { t: Date.now(), ...payload };
      window.localStorage && localStorage.setItem(key, JSON.stringify(cur));
    } catch (e) {}
  }
  ...
  window.addEventListener("error", function(ev) {
    try {
      storeLastError({
        type: "error",
        message: (ev && ev.message) ? String(ev.message).slice(0, 400) : "unknown",
  ...
  window.addEventListener("unhandledrejection", function(ev) {
    try {
      ...
      storeLastError({
        ...
        message: reason ? String(reason && (reason.message || reason)).slice(0, 400) : "unknown"
      });
    } catch (e) {}
  });
})();
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    // Track last user action for debugging (no PII)
    document.addEventListener("click", (e) => {
      try {
        const el = e.target && e.target.closest && e.target.closest("[data-action],button,a,input,label");
        if(!el) return;
        const id = el.id ? `#${el.id}` : "";
        const da = el.getAttribute && el.getAttribute("data-action");
        const name = da || el.getAttribute("aria-label") || el.name || el.type || el.tagName;
        window.__WC_APP_STATE__ = window.__WC_APP_STATE__ || {};
        window.__WC_APP_STATE__.lastAction = String(name || "").slice(0, 80) + id;
      } catch (err) {}
    }, { capture: true });
    ...
    async function renderMonth(canvas, W, H, month, forHit){
      ...
      const headerCount = clamp((ms.headerCount??1), 0, 4);
    ...
    downloadYearBtn.addEventListener("click", async ()=>{
      try{
        saveUIToCurrent();
        /* portada opcional: puede estar en blanco */
      ...
      try{
        ...
        if(cfg.months){
          for(let m=1;m<=12;m++){
            ...
            monthState[m].headerCount = clamp((parseInt(msCfg.headerCount??1,10)||0),0,4);
